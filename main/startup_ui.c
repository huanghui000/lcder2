...
void startup_ui_init(void)
{
    ...
    lv_obj_set_style_text_font(s_title, &ui_font_simli, LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_text_font(s_detail, &ui_font_simliSmall, LV_PART_MAIN | LV_STATE_DEFAULT);
...
void startup_ui_show_connected(const char *detail)
{
    ...
    memset(&state, 0, sizeof(state));
    strncpy(state.title, "网络已就绪", sizeof(state.title) - 1);
    strncpy(state.detail, detail ? detail : "已成功连接网络", sizeof(state.detail) - 1);
...
{
    LV_UNUSED(delay_ms);
    startup_ui_show_connected("已成功连接网络");
}
